...
function Remove-Comments($lines) {
  ...
  foreach ($line in $lines) {
    ...
    #to handle HP Functional Comments as Special objectAtributes
    # --#TYPE         "I2C Errors above critical level	 (UC_GH)"
    # --#SUMMARY      "%s"
    # --#ARGUMENTS    { 1 }
    # --#SEVERITY     CRITICAL
    # --#TIMEINDEX    1
    ...
    #$line = $line -replace '--#', 'HPComment'
    $line = $line -replace '--#TYPE', 'HPComment.TYPE'
    $line = $line -replace '--#SUMMARY', 'HPComment.SUMMARY'
    $line = $line -replace '--#ARGUMENTS', 'HPComment.ARGUMENTS'
    $line = $line -replace '--#SEVERITY', 'HPComment.SEVERITY'
    $line = $line -replace '--#TIMEINDEX', 'HPComment.TIMEINDEX'
    $line = $line -replace '--#HELPTAG', 'HPComment.HELPTAG'
    $line = $line -replace '--#STATE', 'HPComment.STATE'
...
function Get-Tokens($linesWithoutComments) {
  ...
  foreach ($line in $linesWithoutComments) {
    ...
    $line = $line -replace '"', ' " '
    $line = $line -replace '::=', ' ::= '
    # add space between OBJECTS and {
    if ($line -cmatch "OBJECTS{") {
      $line = $line -creplace "OBJECTS{", "OBJECTS {"
    }
...
function Parse-MIB($tokens) {
  ...
  ($HPComment_TYPE, $HPComment_SUMMARY, $HPComment_ARGUMENTS, $HPComment_SEVERITY, $HPComment_TIMEINDEX, $HPComment_HELPTAG, $HPComment_STATE) = ('', '', '', '', '', '', '')
  $imported_modules = ''
  $counter = 0
  ...
  #Adding the HPComments into $expected_type_tokens
  $expected_type_tokens += 'HPComment.TYPE', 'HPComment.SUMMARY', 'HPComment.ARGUMENTS', 'HPComment.SEVERITY', 'HPComment.TIMEINDEX', 'HPComment.HELPTAG', 'HPComment.STATE'  
  ...
  foreach ($token in $tokens) {
    ...
    if ($sa_status -eq 'init') {
      ...
      elseif ($token -eq '::=') {
        ...
        if ( ($tokens[$counter - 1] + $token + $tokens[$counter + 1] -ne 'DEFINITIONS::=BEGIN') -and ($tokens[$counter + 1] -ne 'TEXTUAL-CONVENTION') -and ($tokens[$counter + 1] -ne 'SEQUENCE') ) {
          $sa_status = 'SYNTAX'
    ...
    elseif ($sa_status -eq 'FROM') {
      ...
      ($object_name, $object_type, $object_syntax, $status, $description, $objects, $ID, $object_max_access, $object_units, $object_reference, $object_index, $object_augments, $object_defval, $notification_objects, $parent, $OID) = ('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '')
      $objects = '{ '
      if ($token.endswith(';') -or ($tokens[$counter + 1] -eq ';') ) {
        $imported_modules += '}'
    ...
    elseif ($sa_status -eq 'OBJECT-TYPE') {
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
      }
      $counter += 1
      continue
    ...
    elseif ($sa_status -eq '::=') {
      ...
      #Adding HPComment variables in objectProperties
      $HPCommentsVarNames = "HPComment_TYPE", "HPComment_SUMMARY", "HPComment_ARGUMENTS", "HPComment_SEVERITY", "HPComment_TIMEINDEX", "HPComment_HELPTAG", "HPComment_STATE"
      foreach ($varName in $HPCommentsVarNames) {
        $propertyName = $varName.Replace("HPComment_", "HPComment-")
        $propertyValue = Get-Variable -Name $varName -ValueOnly
      ...
      #clean up all HPComment variables
      foreach ($varName in $HPCommentsVarNames) {
    ...
    #HPComments related statuses
    elseif ($sa_status -eq 'HPComment.TYPE'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_TYPE = $HPComment_TYPE.trim()
      }
      else{
        $HPComment_TYPE += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.SUMMARY'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_SUMMARY = $HPComment_SUMMARY.trim()
      }
      else{
        $HPComment_SUMMARY += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.ARGUMENTS'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_ARGUMENTS = $HPComment_ARGUMENTS.trim()
      }
      else{
        $HPComment_ARGUMENTS += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.SEVERITY'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_SEVERITY = $HPComment_SEVERITY.trim()
      }
      else{
        $HPComment_SEVERITY += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.TIMEINDEX'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_TIMEINDEX = $HPComment_TIMEINDEX.trim()
      }
      else{
        $HPComment_TIMEINDEX += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.HELPTAG'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_HELPTAG = $HPComment_HELPTAG.trim()
      }
      else{
        $HPComment_HELPTAG += $token + " "
      }
      $counter += 1
      continue
    }
    elseif ($sa_status -eq 'HPComment.STATE'){
      if ($expected_type_tokens -contains $token) {
        $sa_status = $token
        $HPComment_STATE = $HPComment_STATE.trim()
      }
      else{
        $HPComment_STATE += $token + " "
      }
      $counter += 1
      continue
    }
    ...
    if ($traps) {
      ...
      foreach ($trap in $traps) {
          ...
          foreach ($object_name in $object_names) {
            ...
            if (!$object) {
              if ($OIDrepo) {
                ...
                if (!$object) {
                  $trap_name = $trap.objectName
                  $errorMessage = "Object: $object_name in trap: $trap_name not resolved; checked the IMPORT section of MIB and update OIDrepo with corresponding MIBs for more details see Get-Help Import-MIB"
                  Write-Error $errorMessage
                  if ($ErrorLog) {
                    Out-File -FilePath $ErrorLog -Append -InputObject $errorMessage
                  }
                }
              }
              else {
        ...
        if ($SnmpVersion -match '2') {
          ...
          $snmp_v1_comment = "#" + $trap.objectName  
...
.EXAMPLE
    $mibsInfo = @(); foreach ($mib in ls *.mib){$mibsInfo += Get-MIBInfo $mib}; $mibsInfo | Export-Csv -NoTypeInformation MibsInfo.csv
    Process all the MIBs and Export the info about them in CSV file for later analyses.

.EXAMPLE
    $mibsInfo = @(); $csvName = ((pwd).ToString().split('\')[-1] -replace '\.','_') + '-MIBInfo.csv'; foreach ($mib in ls *.mib){$mibsInfo += Get-MIBInfo $mib}; $mibsInfo | Export-Csv -NoTypeInformation $csvName
    Process all the MIBs and Export the info about them in CSV file for later analyses. Save in the CVS named based on folder name.

.EXAMPLE
    foreach ($mib in ls *.mib) {$mibinfo = Get-MIBInfo $mib; $newname = $mibinfo.moduleName + '.mib'; echo "$mib -> $newname"; mv $mib $newname }
    Rename all .mib files based on their Module name

.EXAMPLE
    foreach ($mib in ls *.mib) {$mibinfo = Get-MIBInfo $mib; $newname = $mibinfo.moduleName + '_' + $mibinfo.lastUpdated; $newname = $newname.trim('_') + '.mib'; echo "$mib -> $newname"; mv $mib $newname }
    Rename all .mib files based on their Module name and lastUpdate (if defined in mib)    
#>
  ...
  END {
    ...
    Foreach ($token in $tokens) {
      ...
      elseif ($sa_status -eq 'process_imports_from') {
        if ($token.endswith(';') -or ($tokens[$token_counter + 1] -eq ';')) {
          $sa_status = 'init'
      ...
      elseif ($sa_status -eq 'process_revisions') {
        ...
        elseif ($token -eq '::=') {
          ...
          if ($revision_numbers.Length -gt 1) {
            ...
            $latest_revision = $latest_revision -replace '\"', ''
          }
    ...
    (Get-Content -Raw $Path) -replace "`r`n", "`n" | Set-Content -NoNewline LinEOLTemp.txt
    $Linux_file_hash_SHA256 = (Get-FileHash -Algorithm SHA256 LinEOLTemp.txt).Hash
    $Linux_file_hash_MD5 = (Get-FileHash -Algorithm MD5 LinEOLTemp.txt).Hash

    (Get-Content -Raw LinEOLTemp.txt) -replace "`n", "`r`n" | Set-Content -NoNewline WinEOLTemp.txt
    $Win_file_hash_SHA256 = (Get-FileHash -Algorithm SHA256 WinEOLTemp.txt).Hash
    $Win_file_hash_MD5 = (Get-FileHash -Algorithm MD5 WinEOLTemp.txt).Hash

    remove-item -Force LinEOLTemp.txt
    remove-item -Force WinEOLTemp.txt

    $imports_string = '{' + ($imports -join ',') + '}'
    $revisions_string = (((( $revisions | ConvertTo-Json -Compress ) -replace ',', ';') -replace '\\\"', '') -replace '\[', '') -replace '\]', ''
    $objectProperties = @{ fileFullName = $fileInfo.FullName; fileName = $fileInfo.Name; fileSize = $fileInfo.Length; moduleName = $module_name; lastUpdated = $last_updated; revisions = $revisions_string; latestRevision = $latest_revision; description = $description; organization = $organization; contact = $contac_info; LinuxFileHashSHA256 = $Linux_file_hash_SHA256; LinuxFileHashMD5 = $Linux_file_hash_MD5; WinFileHashSHA256 =  $Win_file_hash_SHA256; WinFileHashMD5 = $Win_file_hash_MD5; imports = $imports_string }
...
function Get-MIBBackwardsCompatiblity {
  <#  
.SYNOPSIS  
...
.EXAMPLE
    Get-MIBBackwardsCompatiblity -Newer .\ocum-9_4.csv -Older .\ocum-6_2.csv
    Returns true if ocum-9_4.csv is backward compatible with ocum-6_2.csv 

.EXAMPLE
    $csvs=Get-ChildItem *.csv | sort -Property Name;$index=1;while ( $index -lt $csvs.Length ) { $older = $csvs[$index-1];$newer = $csvs[$index];echo "<-- BEGIN -->" ;echo "older = $older; newer = $newer"; Get-MIBBackwardsCompatiblity -Older $older -Newer $newer -CompatibilityLevel 2 -DetailLevel 2; echo "<--  END  -->"; $index += 1}     
    This can give you Compatibility overview of all files (csvs in this example), but It must be sorted chronologically from oldest to newewst.
    To achieve the same I'm including the version/last updated info in name of the file. Like this:
      PS Z:\\DELL-DataDomain\MIBS\AllVersions> ls *.csv

    Directory: Z:\\DELL-DataDomain\MIBS\AllVersions

Mode                 LastWriteTime         Length Name
----                 -------------         ------ ----
-a----         8/11/2022  11:55 AM         200073 DATA-DOMAIN-201011190000Z.csv
-a----         8/11/2022  11:56 AM         304364 DATA-DOMAIN-201112090000Z.csv
-a----         8/11/2022  11:57 AM         412802 DATA-DOMAIN-201501290000Z.csv

.EXAMPLE
    $newerfolder=".\upd11.80\";  foreach ($mib in ls *.mib) { $olderMib = $mib.name; $newermib = $newerfolder + $mib.name; echo "Newer: $newermib ; Older: $olderMib";echo "<-- BEGIN -->" ; Get-MIBBackwardsCompatiblity -Older $olderMib -Newer $newermib -CompatibilityLevel 2 -DetailLevel 2; echo "<--  END  -->";}
    
    Compares the MIBs from current directory with MIBs from given directory, where given directory is newer

.EXAMPLE
    $newerfolder=".\Dell-OM-MIBS-Universal\"; $olderfolder=".\Dell-OM-MIBS-850_A00\"; foreach ($mib in ls "$olderfolder\*.csv") { $olderMib = $olderfolder + $mib.name; $newermib = $newerfolder + $mib.name; echo "Newer: $newermib ; Older: $olderMib";echo "<-- BEGIN -->" ;Get-MIBBackwardsCompatiblity -Older $olderMib -Newer $newermib -CompatibilityLevel 2 -DetailLevel 2; echo "<--  END  -->";}

    Compares the corresponding MIBs in given directories

.EXAMPLE
    $newerfolder=".\Dell-OM-MIBS-Universal\"; $olderfolder=".\Dell-OM-MIBS-850_A00\"; $repoCsv="Dell-iDrac.csv"; foreach ($mib in ls -Exclude $repoCsv "$olderfolder\*.csv") { $olderMib = $olderfolder + $mib.name; $newermib = $newerfolder + $mib.name; echo "Newer: $newermib ; Older: $olderMib";echo "<-- BEGIN -->" ;Get-MIBBackwardsCompatiblity -Older $olderMib -Newer $newermib -CompatibilityLevel 2 -DetailLevel 2; echo "<--  END  -->";}
    
    Compares the corresponding MIBs in given directories, excluding repo file
#>
    ...
    try {
      if ($Newer) {
        ...
        elseif ($newerFile.Extension -eq '.csv') {
          ...
          "" 
          get-help Get-MIBBackwardsCompatiblity
        }
      }
      else {
      ...
      if ($diffObject.sideIndicator -eq '=>') {
        ...
        if ($DetailLevel -gt 0) { 
          ...
          if ($newerObject) {
            ...
            if ($newerObject.description -eq $diffObject.description) {
              $detailLevelName = 'WARNING: '
              $detailMessage = $detailLevelName + 'Object with same OID and diferent name(possibly name of object change), probably just rename of object,cause description is same: ' + $diffObject.objectFullName 
            }
        ...
        if ($olderObject) {
          ...
          if ($olderObject.OID -eq $diffObject.OID) {
            ...
            if ($CompatibilityLevel -gt 1) {
              ...
              if ($DetailLevel -gt 1) {
                $detailLevelName = 'ERROR: '
                $detailMessage = $detailLevelName + 'OID changed: Older: ' + $olderObject.OID + '(' + $olderObject.objectFullName + ') =>  Newer: ' + $diffObject.OID + '(' + $diffObject.objectFullName + ')'
                # DEBUG BEGIN
                # Write-Output "= Older ======================================================"
                # $olderObject
                # $olderObject |measure
  ...
  END {
    ...
    $missingObjects | ConvertTo-SMIv1 | Out-File missing-objects-SMIv1.mib
    $missingObjects | ConvertTo-SMIv2 | Out-File missing-objects-SMIv2.mib
    return $compatible
  }
...
.EXAMPLE
    ConvertTo-SMIv2 -Path .\NIMBLE.csv
    Convert CSV file generated by Import-MIB. !NOTE: use | Export-Csv -UseCulture otherwise it will not work.
...
.EXAMPLE
  ...
  END {
    $TextToPrint = '-- THIS MIB IS GENERATED AUTOMATICALLY BY ConvertTo-SMIv2'
    Write-Output $TextToPrint
    Write-Output ''
    ...
    foreach ($object in $mib) {
      if ($object.objectType -eq 'IMPORTS') {
        ...
        Write-Output '-- IMPORT IS YET NOT IMPLEMENTED IN THI VERSION'
        Write-Output 'IMPORTS'
        #have to add NOTIFICATION-TYPE definition for proper traps definitions
        Write-Output '    MODULE-IDENTITY, OBJECT-TYPE, NOTIFICATION-TYPE, enterprises, Counter32,Integer32'         
        Write-Output '        FROM SNMPv2-SMI'
        Write-Output '-- Add copy of import section from original MIB file'
      ...
      elseif ($object.objectType -eq 'OBJECT-TYPE') {
        ...
        $TextToPrint = '  SYNTAX ' + $object.objectSyntax  
        Write-Output $TextToPrint 
        $TextToPrint = '  MAX-ACCESS ' + $object.maxAccess 
        Write-Output $TextToPrint 
        $TextToPrint = '  STATUS ' + $object.status 
        Write-Output $TextToPrint
        $TextToPrint = '  DESCRIPTION ' + $object.description 
      ...
      elseif ($object.objectType -eq 'TEXTUAL-CONVENTION') {
        ...
        if ( $object.description -eq '' ) {
          $TextToPrint = '  DESCRIPTION "No description"' 
          Write-Output $TextToPrint
        }
        else {
          $TextToPrint = '  DESCRIPTION ' + $object.description 
...
#20220728,CORRECTED: standalone semicolon(;) in IMPORTS was ignored, ADDED: BEGIN, END and partial IMPORTS in Convertto-SMIv2
#20220811,ADDED: Example for Is-Backward compatible to process more files at once
#20221124,CHANGED: Get-MIBInfo not returns lastUpdated and latestRavision enclosed in Qoutes("), bacause the if remove the zne at the end can b compared as numbers using >,<,=
#20230102,ADDED: Is-BackwardsCompatible function generates "MIB" with Missing objects - this may solve compatibilty in cases where only object from old mib are missing in new MIB
#20230106,CORRECTED: Get-MIBInfo was not returning MODULE-IDENTITY data when Last Imported module was not ending with ; but ; stand separetly like INET-ADDRESS-MIB ;
#20230214,ADDED: print comment with name of trap for snmptrap v1
#20230226,ADDED: Get-MIBInfo added more Examples in help 
#20230401,ADDED: New example to Is-BackwardsCompatible
#20230408,CORRECTED: Remove-Comments didn't hadndle closing -- in comments; CORRECTED: Get-MIBInfo not getting revisions
#20230410,ADDED: Better error message in Convertto-snmptrap
#20230417,ADDED: More examples in Is-BackwardsCompatible
#20230420,ADDED: Addition example in Is-BackwardsCompatible and corrected one example
#20230503,ADDED: ErrorLog option in function ConvertTo-Snmptrap
#20230518,ADDED: new fields in Get-MIBInfo output WinFileHashSHA256, LinuxFileHashMD5, LinuxFileHashSHA256, WinFileHashMD5
#20230612,ADDED: Parse-MIB to parse HPComments + code optimizations; Remove-Comments to transform HPComments into Tokens;
#20231117,CHANGED: Renamed Is-BackwardsCompatible to Get-MIBBackwardsCompatiblity, ADDED: Warning instead of error if Name of OID chnaged but description is same
#20231123,ADDED: Get-Tokens extra check and replace for space between OBJECTS and {